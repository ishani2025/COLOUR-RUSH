...
var shark=null,waterTime=0,sharkWarned=false,sharkAng=0,sharkAttacking=false,sharkAttackT=0;
var _tmp=new THREE.Vector3(), _ONE=new THREE.Vector3(1,1,1), _tiny=new THREE.Vector3(0.01,0.01,0.01);
var moveX=0,moveZ=0,idleT=0;
var dragon=null,dragonActive=false,dragonT=0,dragonShadow=null,dragonClouds=null;
...
function initJoystick(){
  var j=el('joystick'), s=el('stick'); if(!j) return;
  var isTouch=('ontouchstart' in window)||navigator.maxTouchPoints>0;
  if(isTouch) j.style.display='block';
  var id=null,R=54,cx=0,cy=0;
  ...
  j.addEventListener('touchend',function(e){ for(var i=0;i<e.changedTouches.length;i++){ if(e.changedTouches[i].identifier===id) end(); } });
  j.addEventListener('touchcancel',end);
  j.addEventListener('mousedown',function(e){ setC(); mv(e.clientX,e.clientY); function mm(ev){ mv(ev.clientX,ev.clientY);} function mu(){ end(); removeEventListener('mousemove',mm); removeEventListener('mouseup',mu);} addEventListener('mousemove',mm); addEventListener('mouseup',mu); });
}
...
var LOW=false;
function init(){
  LOW=(('ontouchstart' in window)||navigator.maxTouchPoints>0)||Math.min(innerWidth,innerHeight)<820;
  scene=new THREE.Scene();
  scene.background=new THREE.Color(0x9fd0ea);
  scene.fog=new THREE.Fog(0x9fd0ea, 60, LOW?95:150);
  camera=new THREE.PerspectiveCamera(60, innerWidth/innerHeight, 0.1, LOW?210:700);

  renderer=new THREE.WebGLRenderer({antialias:!LOW});
  ...
  sun.position.set(60,90,40); sun.castShadow=!LOW;
...
function buildShark(){
  ...
  var grey=new THREE.MeshStandardMaterial({color:0x808b96, roughness:.6, metalness:.05});
  var belly=new THREE.MeshStandardMaterial({color:0xe2e7ea, roughness:.6});
  var dark=new THREE.MeshStandardMaterial({color:0x141a1f, roughness:.5});
  var pink=new THREE.MeshStandardMaterial({color:0xbf6b72, roughness:.6});
  var white=new THREE.MeshStandardMaterial({color:0xffffff, roughness:.3});
  // huge megalodon body
  var body=sh(new THREE.Mesh(new THREE.SphereGeometry(1,22,16), grey)); body.scale.set(4.6,1.5,1.9); shark.add(body);
  var bel=new THREE.Mesh(new THREE.SphereGeometry(1,22,16), belly); bel.scale.set(4.3,1.0,1.7); bel.position.y=-0.5; shark.add(bel);
  var snout=sh(new THREE.Mesh(new THREE.ConeGeometry(1.3,2.8,18), grey)); snout.rotation.z=-Math.PI/2; snout.position.set(4.9,0.55,0); shark.add(snout);
  // upper teeth
  for(var i=0;i<11;i++){ var ut=new THREE.Mesh(new THREE.ConeGeometry(0.13,0.45,4), white); ut.position.set(4.1,-0.15,-1.0+i*0.2); ut.rotation.x=Math.PI; shark.add(ut); }
  // lower jaw (opens during the attack)
  var jaw=new THREE.Group(); jaw.position.set(3.6,-0.6,0); shark.add(jaw); shark.userData.jaw=jaw;
  var jbase=sh(new THREE.Mesh(new THREE.BoxGeometry(2.8,0.45,2.6), dark)); jbase.position.set(0.5,0,0); jaw.add(jbase);
  var mouthIn=new THREE.Mesh(new THREE.BoxGeometry(2.6,0.7,2.2), pink); mouthIn.position.set(0.5,0.4,0); jaw.add(mouthIn);
  for(var j=0;j<11;j++){ var lt=new THREE.Mesh(new THREE.ConeGeometry(0.13,0.48,4), white); lt.position.set(1.4,0.6,-1.0+j*0.2); jaw.add(lt); }
  // dorsal fin
  var s=new THREE.Shape(); s.moveTo(-1.1,0); s.lineTo(1.1,0); s.quadraticCurveTo(0.5,1.9,-0.4,2.2); s.quadraticCurveTo(-0.8,1.1,-1.1,0);
  var dorsal=sh(new THREE.Mesh(new THREE.ExtrudeGeometry(s,{depth:0.24,bevelEnabled:false}), grey)); dorsal.position.set(-0.3,1.25,-0.12); shark.add(dorsal);
  var pec=sh(new THREE.Mesh(new THREE.ConeGeometry(0.7,2.7,4), grey)); pec.scale.set(1,1,0.3); pec.rotation.z=Math.PI/2; pec.position.set(1.0,-0.45,1.7); shark.add(pec);
  var pec2=sh(new THREE.Mesh(new THREE.ConeGeometry(0.7,2.7,4), grey)); pec2.scale.set(1,1,0.3); pec2.rotation.z=Math.PI/2; pec2.position.set(1.0,-0.45,-1.7); shark.add(pec2);
  var tailT=sh(new THREE.Mesh(new THREE.ConeGeometry(0.8,3.4,4), grey)); tailT.scale.set(0.55,1,0.26); tailT.position.set(-5.0,1.1,0); tailT.rotation.z=0.5; shark.add(tailT);
  var tailB=sh(new THREE.Mesh(new THREE.ConeGeometry(0.65,2.2,4), grey)); tailB.scale.set(0.55,1,0.26); tailB.position.set(-4.9,-0.7,0); tailB.rotation.z=-0.4; shark.add(tailB);
  var e1=new THREE.Mesh(new THREE.SphereGeometry(0.18,8,8), dark); e1.position.set(3.6,0.55,1.15); shark.add(e1);
  var e2=new THREE.Mesh(new THREE.SphereGeometry(0.18,8,8), dark); e2.position.set(3.6,0.55,-1.15); shark.add(e2);
...
function buildDragon(){
  ...
  dragonClouds=new THREE.Group();
  for(var ci=0;ci<8;ci++){ var pf=new THREE.Mesh(new THREE.SphereGeometry(2.4+Math.random()*1.6,10,8), new THREE.MeshBasicMaterial({color:0xeef2f7, transparent:true, opacity:0.9})); pf.position.set((Math.random()-0.5)*18,(Math.random()-0.5)*7,(Math.random()-0.5)*18); pf.scale.y=0.55; dragonClouds.add(pf); }
  dragonClouds.visible=false; scene.add(dragonClouds);
}
function playerInOcean(){ return Math.hypot(player.position.x,player.position.z) > 93; }
function dieOrDragon(msg){ if(phaseFor(attempt)>=4 && dragon && idleT>0.8){ dragonDeath(); } else { failTrial(msg); } }
...
// ================= GAME FLOW =================
function startGame(){
  ensureAudio(); startMusic(); score=0; attempt=0; lastPhase=0; lastSafeColor=null; baseLavaY=-6; monsterActive=false; if(monster)monster.visible=false; waterTime=0; sharkWarned=false; dragonActive=false; if(shark)shark.visible=false; if(dragon)dragon.visible=false; sharkAttacking=false; idleT=0; moveX=0; moveZ=0; if(dragonClouds)dragonClouds.visible=false; if(dragonShadow)dragonShadow.visible=false;
...
function animate(){
  ...
  if(state==='playing'||state==='prep'){
    ...
    var rx=mx+touchMove.x, rz=mz+touchMove.z, rmag=Math.hypot(rx,rz);
    if(rmag>1){ rx/=rmag; rz/=rmag; }
    moveX+=(rx-moveX)*Math.min(1,dt*9); moveZ+=(rz-moveZ)*Math.min(1,dt*9);   // smoothed, controllable
    var mag=Math.hypot(moveX,moveZ);
    if(mag>0.05){ var nx=moveX/mag, nz=moveZ/mag;
      var sp=PLAYER_SPEED*(LOW?0.62:1)*Math.min(1,mag);   // analog: partial joystick push = partial speed
      ...
      idleT=0;
    } else if(state==='playing'){ idleT+=dt; }
  ...
  // MEGALODON attack: lunges up from the deep with jaws open
  if(sharkAttacking && shark){
    sharkAttackT+=dt; var aq=Math.min(1,sharkAttackT/1.4);
    shark.visible=true; shark.scale.copy(_ONE);
    shark.rotation.set(0,0,Math.PI/2);
    shark.position.set(player.position.x, THREE.MathUtils.lerp(-18,-3,es(aq)), player.position.z);
    if(shark.userData.jaw) shark.userData.jaw.rotation.z=-0.9*Math.min(1,aq*1.4);
    shake=Math.max(shake,0.75);
    if(aq>=1){ sharkAttacking=false; flash('DEVOURED BY THE MEGALODON!', '#3fa9ff'); setTimeout(gameOver,650); }
  }
  else if(state==='playing' && shark){
    if(playerInOcean()){ waterTime+=dt;
      ...
      shark.scale.lerp(_ONE,0.09);
      sharkAng+=dt*(0.7+waterTime*0.14);
      var rad=Math.max(5, 13-waterTime*1.1);
      var tx=player.position.x+Math.cos(sharkAng)*rad, tz=player.position.z+Math.sin(sharkAng)*rad;
      shark.position.lerp(_tmp.set(tx, 0.6+Math.sin(t*2.2)*0.14, tz), 0.09);
      shark.rotation.set(0, -Math.atan2(Math.cos(sharkAng), -Math.sin(sharkAng)), 0);
      if(shark.userData.jaw) shark.userData.jaw.rotation.z=0;
      if(waterTime>3 && !sharkWarned){ sharkWarned=true; flash('MEGALODON! GET OUT OF THE WATER!', '#3fa9ff'); }
      if(waterTime>=6){ sharkAttacking=true; sharkAttackT=0; state='resolving'; growl(); }
    } else { waterTime=0; sharkWarned=false; if(shark.visible){ shark.scale.lerp(_tiny,0.18); if(shark.scale.x<0.05) shark.visible=false; } }
  } else if(shark && state!=='playing' && !sharkAttacking){ shark.visible=false; }

  // dragon dive (Round 4 fail)
  // Round 3: an ominous dragon shadow drifts overhead (warning of the dragon)
  if(dragonShadow && !dragonActive){
    if(lastPhase===3 && state==='playing'){
      dragonShadow.visible=true;
      var ox=player.position.x+Math.sin(t*0.5)*24, oz=player.position.z+Math.cos(t*0.35)*24;
      dragonShadow.position.set(ox, th(ox,oz)+0.14, oz);
      var osz=5+Math.sin(t*0.9)*1.6; dragonShadow.scale.set(osz,osz,osz);
      dragonShadow.material.opacity=0.26+Math.sin(t*0.9)*0.1;
    } else if(lastPhase!==3){ dragonShadow.visible=false; }
  }
  if(dragonActive && dragon){
    ...
    if(P<0.6){ var k=es(P/0.6);
      ...
      var fly=dgy+3.5+k2*46;
      dragon.position.set(dpx, fly, dpz);
      dragon.lookAt(dpx, fly-6, dpz);
      player.position.set(dpx+Math.sin(t*3)*0.3, fly-1.7, dpz);   // carried in the jaws
      if(dragonClouds){ dragonClouds.visible=fly>20; dragonClouds.position.set(dpx, fly+1.5, dpz); dragonClouds.rotation.y+=dt*0.25; }
      shake=Math.max(shake,0.6);
    } else {
      dragonActive=false; dragon.visible=false; if(dragonShadow)dragonShadow.visible=false; if(dragonClouds)dragonClouds.visible=false;
      flash('EATEN BY THE DRAGON!', '#ff7a1f'); setTimeout(gameOver, 700);
    }
  }
  ...
  var lookY = (dragonActive && dragon)? Math.min(dragon.position.y-2, 26) : 1.4;
  camera.lookAt(ct.x, lookY, ct.z);
